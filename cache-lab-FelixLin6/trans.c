...
// stride will be constantly 8 because writeup specifies graded cases will
// always have b = 6. Thus, a block is 64 bytes, or 8 doubles.
...
static void transpose_submit(size_t M, size_t N, double A[N][M], double B[M][N],
    ...
    if (M == N) {
        for (size_t i = 0; i < N; i += STRIDE) {
            for (size_t j = 0; j < M; j += STRIDE) {
                for (size_t bi = i; bi < i + STRIDE && bi < N; bi++) {
                    for (size_t bj = j; bj < j + STRIDE && bj < M; bj++) {
                        if (bi != bj) {
                            B[bj][bi] = A[bi][bj];
                        }
                    }
                    // If two elements are on diagonal within a block, but are
                    // also on the diagonal blocks in a matrix, we transpose
                    // outside forloop to reduce evictions
                    if (i == j) {
                        B[bi][bi] = A[bi][bi];
                    }
                }
    ...
    // Matrix isn't a square, thus we cannot assume there is a diagonal block
    // given a i or j
    else {
        for (size_t i = 0; i < N; i += STRIDE) {
            for (size_t j = 0; j < M; j += STRIDE) {
                for (size_t ii = i; ii < i + STRIDE && ii < N; ++ii) {
                    for (size_t jj = j; jj < j + STRIDE && jj < M; ++jj) {
                        B[jj][ii] = A[ii][jj];
                    }
                }
            }
